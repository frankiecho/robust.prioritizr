...
#'
#' @description
#' This super-class is used to construct objects.
...
ConservationModifier <- R6::R6Class(
  ...
  public = list(
    ...
    #' the object.
    #' @param x [`prioritizr::OptimizationProblem-class`] object.
    #' @param y [prioritizr::problem()] object.
    ...
    #' then a [prioritizr::new_waiver()] object is returned.
    get_data = function(x) {
      if (!x %in% names(self$data)) return(prioritizr::new_waiver())
      self$data[[x]]
    },
    ...
    get_internal = function(x) {
      if (!x %in% names(self$internal)) return(prioritizr::new_waiver())
      self$internal[[x]]
    },
