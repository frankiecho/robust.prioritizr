...
rs <- c(rsv1, rsv2, rsv3, rsv1-pa, rsv2-pa, rsv3-pa)
names(rs) <- c("Robust (1% target)", "Partially Robust (7% target)", "Fully Robust (7% target)", "New PA (Robust 1%)", "New PA (Partially Robust 7%)", "New PA (Fully Robust 7%)")
plot(rs, axes = F, fun = \() lines(study_area), type = 'continuous', range = c(0,1))
```
...
```{r}
cost_nr <- eval_cost_summary(rpv2, sv1) 
cost_r <- eval_cost_summary(rpv2, rsv2) 
cost_fr <- eval_cost_summary(rpv2, rsv3)

rs <- c(sv1, sv1-pa, rsv3, rsv1-pa, rsv2-pa, rsv3-pa)
names(rs) <- c("Not robust", "Partially Robust", "Fully Robust", "New PA (Not Robust)", "New PA (Partially Robust)", "New PA (Fully Robust 7%)")
plot(rs, axes = F, fun = \() lines(study_area), type = 'continuous', range = c(0,1))
```
