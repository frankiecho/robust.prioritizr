...
vignette: >
  %\VignetteIndexEntry{Example using Victoria, Australia}
...
if (identical(Sys.getenv("PKGDOWN_BUILD"), "true")) {
  is_check <- FALSE
}
knitr::opts_chunk$set(
  fig.align = "center",
...
```{r, fig.alt = "Map of Victoria, Australia, showing the robust conservation planning solution with a 0.45% target."}
rt <- .0045
...
Users can control the robustness of the solution through the `conf_level` parameter in `add_*_robust_constraints`, which specifies the proportion of the constraints that must be met in each group for the solution to be considered feasible. In this case, specifying 75% means that out of the 16 historic and future climate projections, the solution needs to meet the target in at least 12 (16 \\times 0.75) climate projections/ time-steps. In other words, the target can only be breached for 4 climate projections/ time-steps. By relaxing this target, we can pursue much more ambitious relative targets (25% protection), while ensuring that the solution is feasible and reasonably robust under most climate projections and time-steps.

```{r, fig.alt = "Side-by-side maps of Victoria, Australia, comparing the robust and partially robust conservation planning solutions."}
rt2 <- 0.25
...
```{r, fig.alt = "Three maps of Victoria, Australia, comparing the robust, partially robust, and fully robust* conservation planning solutions."}
...
For comparison, we solve a standard `prioritizr` problem using only the historical baseline data and a higher target. We apply the same 25% target to the problem, but only solve this using the historical baseline data.

```{r, fig.alt = "Side-by-side maps of Victoria, Australia, comparing the non-robust and partially robust conservation planning solutions."}
...
Add the CVaR solution here
```{r}
var_rob_cons_cvar <- var_rob_cons
var_rob_cons_cvar$conf_level <- if_else(unique(groups) %in% relax_cons_species, 0.5, 0.9)
...
csv4 <- compile(rpv4)
A <- csv4$A()
row_ids <- csv4$row_ids()
col_ids <- csv4$col_ids()
lb <- csv4$lb()
ub <- csv4$ub()
rhs <- csv4$rhs()

lb[str_detect(col_ids, "cvar_s")]
ub[str_detect(col_ids, "cvar_s")]
...
image(A[str_detect(row_ids, "cvar"), str_detect(col_ids, "cvar")])

sol <- rpv4$portfolio$run(csv4, rpv4$solver)
lhs <- as.vector(csv4$A() %*% sol[[1]]$x)
check <- data.frame(lhs = lhs,
sense = csv4$sense(),
rhs = csv4$rhs(),
diff = lhs - csv4$rhs())
sum(check$diff + 1e-10 < 0)

```

```{r}

feature_rep_r_cvar <- eval_feature_representation_summary(rpv2, sv1) %>%
  mutate(groups = groups) %>%
  group_by(groups) %>%
  summarise(
    target = mean(rt2*total_amount),
    cvar_held = mean(relative_held[relative_held <= quantile(relative_held, 0.5)]),
    cvar_held_absolute = mean(absolute_held[absolute_held <= quantile(absolute_held, 0.5)]),
  ) %>%
    mutate(cvar_held_pct = cvar_held_absolute / target,
     met_cvar_target = cvar_held_absolute >= target)
```
...
row_id <- csv4$row_ids()
col_id <- csv4$col_ids()
image(A[str_detect(row_id, "cvar"), str_detect(col_id, "cvar")])
A[str_detect(row_id, "spp_target"), str_detect(col_id, "pu")] %>% rowSums()
rhs[str_detect(row_id, "spp_target")]
```

Check the size of the new protected area

```{r}
net_area = list(sv1, rsv2, rsv3) %>%
  sapply(function(x) {
    global(x - pa, fun = 'sum', na.rm = T)[[1]]
  })
