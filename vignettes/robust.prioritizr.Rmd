...
```{r}
sim_features_robust_raster <- c(sim_feature_1[[1]], sim_feature_1[[2]], rast(feature_2))
names(sim_features_robust_raster) <- c('sim_feature_1_1', 'sim_feature_1_2', 'sim_feature_2')
feature_groupings <- c('f1', 'f1', 'f2')

p2 <- problem(sim_pu_raster, sim_features_robust_raster) %>%
  add_robust_constraints(feature_groupings = feature_groupings) %>%
  add_absolute_targets(20) %>%
