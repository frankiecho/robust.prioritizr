...
```{r, fig.alt="Features and cost of the prioritization problem"}
set.seed(500)
...
```{r, fig.alt = "Ranking of feature 1 values and its prioritization outcomes"}
df <- data.frame(
...
```{r, fig.alt="Feature 1 representation evaluated across the distribution"}
...
```{r, fig.alt="Prioritization outcome of the non-robust and robust solutions"}
...
```{r, fig.alt="Faceted histogram comparing the distribution of feature representation for the not-robust and robust solutions. The robust solution shows a much lower violation rate."}
...
```{r, fig.alt="Prioritization outcome of the robust minimum shortfall objective"}
budget <- target*2
...
```{r, fig.alt="Histogram of feature 1 representation, showing the distribution of outcomes, with bars colored based on whether they are below or above the target. Vertical lines indicate the target and the expected target."}
...
```{r, fig.alt="Map of the robust minimum shortfall objective solution."}
...
```{r, fig.alt="Map comparing the non-robust and robust solutions with a small budget."}
# Use a small budget
small_budget <- 5
...
```{r, fig.alt="Faceted histogram comparing the distribution of feature representation for the not-robust and robust solutions with a small budget. The distributions are similar due to the tight budget constraint."}
