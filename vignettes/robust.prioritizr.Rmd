...
If `prioritizr` overwhelmingly selects planning units with overpredicted features, we can be over-optimistic about the targets we can achieve with our planning solution. This means that there is a good chance that the target will be violated if we get a different draw from the distribution.

To see this, we generate 1000 realisations of feature_1 from the same distribution and evaluate how good our solution is across each of these realisations. We can estimate how likely it is that our solution violates the target constraint.
...
The green vertical line depicts where we expected `feature_1` to be, and the orange bars show what `feature_1` would actually be.

We can see that the planning solution identified violated the target for `feature_1` in most if not all of the realizations we draw from the statistical distribution. Why did the planning solution disproportionately violate the constraint, even though it was identified using an unbiased estimate of `feature_1`? This is because in the prioritization process, the planning units with an overestimate of `feature_1` would be disproportionately selected, as it appears that these planning units have more features. 

In what operations researchers call the "Curse of Optimality", prioritization could amplify errors that are present in the data, selectively choosing planning units with overestimated features or underestimated costs. If robust approaches are not used, the planning solution is likely to violate targets, leading to disappointment in the end result of the prioritization process.
