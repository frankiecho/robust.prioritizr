...
For simplicity, let's suppose the data analyst fitted the following statistical model of the species abundance based on data:
...
plot_solution_eval <- function(soln) {
  ...
  data.frame(value = representation_rs1) %>%
    mutate(name = names(representation_s1)) %>%
    mutate(name = factor(name, names(representation_s1), names(representation_s1))) %>%
...
```{r}
conf_level_vect <- seq(0.5, 1, 0.05)
