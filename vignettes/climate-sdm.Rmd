...
vignette: >
  %\VignetteIndexEntry{Example using simulated data from a species distribution model}
...
```{r, fig.alt = "Line plot showing the expected abundance of Species 1 and Species 2 as a function of temperature."}
set.seed(19895894)
N <- 10
...
```{r, fig.alt = "Raster showing temperature, precipitation and abundance of the two species"}
...
```{r, fig.alt="Species distributions under different scenarios"}
...
```{r,  fig.alt="Species distributions under different scenarios and cost"}
...
```{r, fig.alt="Planning solution using standard prioritizr"}
target <- 20
...
```{r,  fig.alt="Evaluation of species representation across scenarios"}
...
$$ \min \sum_{i=1}^{I} x_i c_i $$
$$ \text{subject to} $$
$$ \Pr_k \{ \sum_{i=1}^I x_i r_{ijk} \geq T_j \} \geq \alpha \quad \forall \space j \in J $$
$$ x_i \in \{0,1\} \quad \forall \space i \in I $$
...
```{r, fig.alt="Planning solution of the non-robust and robust approaches"}
...
```{r, fig.alt="Evaluation of feature representation of the robust approach"}
...
```{r, fig.alt="Evaluation of feature representation of the solution of taking the most extreme climate scenario"}
...
```{r, fig.alt="Feature representation of solution of adding a buffer to the target"}
...
A common misconception to why robust optimization does under the hood (one that I had in the past) was that solving a problem with a robust approach is equivalent to solving the problem while supplying the minimum value of the abundance across all scenarios. Such approaches are common in so-called "climate refugia" analyses, where the objective is to identify overlaps in species ranges across multiple climate scenarios and direct conservation efforts to those areas.
...
```{r, fig.alt="Map of species 2 under different climate scenarios"}
...
```{r, fig.alt="Feature representation of taking the minimum value out of scenarios"}
...
```{r, fig.alt="Map of Species 1 under different climates"}
...
```{r, fig.alt="Feature representation across data replicates"}
...
If we relax the confidence level a bit, we can allow for the target to not be met in some realizations of uncertainty. This allows the optimization problem to search for a lower cost solution. For example, we can specify the `conf_level=0.9`, which mandates that the solution meet the objective in at least 90% of the random realizations supplied to the problem.

```{r, fig.alt="Feature representation across data replicates, with a lower confidence level"}
...
```{r, fig.alt="Cost trade-off between cost and robustness"}
...
```{r, fig.alt="Computational solve time trade-off with robustness relaxation"}
chance_cons_df %>%
